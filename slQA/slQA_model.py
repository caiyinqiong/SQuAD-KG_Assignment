...
from .slQA_network import SLQA_Network


class SLQA_Model(object):
    def __init__(self, opt, embedding):
        self.opt = opt
        self.updates = 0   ## 记录一个epoch训练了多少batch
        self.eval_embed_transfer = True  
        self.train_loss = AverageMeter()
        self.network = SLQA_Network(opt, embedding)  
    ...
    def setup_eval_embed(self, eval_embed):  ## 
        self.network.encoder.eval_embed = nn.Embedding(eval_embed.size(0), eval_embed.size(1), padding_idx=0)
        self.network.encoder.eval_embed.weight.data = eval_embed
        for p in self.network.encoder.eval_embed.parameters():
    ...
    def update_eval_embed(self):  
        if self.opt['embed_tune_partial'] > 0:
            ...
            self.network.encoder.eval_embed.weight.data[0:offset] = self.network.encoder.embedding.weight.data[0:offset]
    ...
    def update(self, batch):
        ...
        loss = F.cross_entropy(start, y[0]) + F.cross_entropy(end, y[1])  ## span   
    ...
    def reset_embeddings(self):
        if self.opt['embed_tune_partial'] > 0:
            offset = self.opt['embed_tune_partial']
            if offset < self.network.encoder.embedding.weight.data.size(0):
                self.network.encoder.embedding.weight.data[offset:] = self.network.encoder.fixed_embedding  
    ...
    def predict(self, batch):
        ...
        for i in range(start.size(0)):  # batch_size
            # label
